...
def adata_preprocess_hvg(adata, n_top_genes):
    ...
    sc.pp.log1p(adata)
    data_matrix = adata[:, adata.var['highly_variable']].X
...
def get_predicted_results(dataset, name, name_annot, path, z, refine_choice):
    ...
    if dataset=="SpatialLIBD":
        ...
        return ari, refined_preds
            ...
            adata = sc.read_h5ad(os.path.join(path, f"{name}_2.h5ad"))
        ...
        if refine_choice == "yes":
            ...
            ari = adjusted_rand_score(label[label != -1], raw_preds)
            return ari, raw_preds

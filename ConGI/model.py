...
class SpaCLR(nn.Module):
    def __init__(self, gene_dims, image_dims, p_drop, n_pos, backbone='densenet', projection_dims=[64, 64]):
        ...
        if backbone == 'densenet':
            ...
            print(f"n_pos: {n_pos}, type: {type(n_pos)}")
            print(f"n_features: {n_features}, type: {type(n_features)}")
...
class TrainerSpaCLR:
    def __init__(self, args, n_clusters, network, optimizer, log_dir, device='cuda'):
        ...
        elif args.dataset == "Space_Ranger_DB":
            adata = sc.read_h5ad(os.path.join(args.path, f"{args.name}_2.h5ad"))
            label = adata.obs['annotation']
            # Convert label to an ordered categorical
            label = pd.Categorical(label, ordered=True)
            label = label.codes
            adata = adata[label != -1]
            self.sample_id = adata.obs.index.tolist()
            self.adj_2d = calculate_adj_matrix(x=adata.obs["array_row"].tolist(), y=adata.obs["array_col"].tolist(), histology=False)
        elif args.dataset == "Space_Ranger_DB2":
            adata = load_ST_file(os.path.join(args.path, args.name))
            df_meta = pd.read_csv(os.path.join(args.path, args.name, args.name_annot), sep=',')
    ...
    def train(self, trainloader, epoch):
        ...
        with tqdm(total=len(trainloader)) as t:
            ...
            for i, batch in enumerate(trainloader):
                ...
                with autocast():  # Utilisation d'autocast pour l'entraînement en précision mixte
                ...
                scaler.scale(loss).backward()  # Backpropagation avec le scaler
                scaler.step(self.optimizer)  # Optimisation avec le scaler
                scaler.update()  # Mise à jour du scaler

                train_cnt += 1
                train_loss += loss.item()

                # Libération de la mémoire GPU pour éviter les fuites de mémoire
                del xg, xg_u, xg_v, xi_u, xi_v, spatial, zg, hg, zi_a, hi_a, zi_b, hi_b, zg_u, hg_u, zg_a, hg_a, zg_b, hg_b, zi_c, hi_c, loss
